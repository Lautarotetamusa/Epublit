...
import { PostLibro, GetPersonas } from "../ApiHandler";
...
const NuevoLibro = ({ personas }) => {
  ...
  const handleIlustradorDelete = ({ dni }) => {
    setIlustradores(
      ilustradores.filter((ilustrador) => ilustrador.dni !== dni)
    );
  };
  ...
  const handleSaveIlustrator = (ilustrator) => {
    if (
      selectedIlustrators.some((ilustrador) => ilustrador.id === ilustrator.id)
    ) {
      Swal.fire({
        title: "Advertencia",
  ...
  const handleExistentesDelete = ({ id, type }) => {
    if (type === "autor") {
      ...
      setSelectedIlustrators(
        selectedIlustrators.filter((ilustrador) => ilustrador.id !== id)
      );
    }
  };
          ...
          {autores.map((autor) => (
            <div key={autor.dni}>
              ...
              <button
                ...
                aria-label="Close"
                onClick={() => handleAutorDelete({ dni: autor.dni })}
              />
            </div>
          ))}
          {selectedAuthors.map((autor) => (
            <div key={autor.id}>
              ...
              <button
                ...
                aria-label="Close"
                onClick={() =>
                  handleExistentesDelete({ id: autor.id, type: "autor" })
                }
              />
            </div>
          ))}
          ...
          {ilustradores.map((ilustrador) => (
            <div key={ilustrador.dni}>
              ...
              <button
                ...
                className="btn-close align-middle"
                aria-label="Close"
                onClick={() => handleIlustradorDelete({ dni: ilustrador.dni })}
              />
            </div>
          ))}
...
export const CrearLibro = () => {
  ...
  return <NuevoLibro personas={personas} />;
};
